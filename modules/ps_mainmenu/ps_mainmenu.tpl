...
{function name="menu" nodes=[] depth=0 parent=null}
  {if $nodes|count}
    <ul {if $depth === 0}class="main-menu__dropdown js-main-menu h-100"{else} class="main-menu__list main-menu__list--{$depth}"{/if} data-depth="{$depth}">
      {foreach from=$nodes item=node}
        ...
        <li class="h-100 main-menu__item--{$depth} {$node.type} main-menu__item {if $depth === 0}main-menu__item--top{else}main-menu__item--sub{/if}{if $node.current} main-menu__item--current{/if}"
          ...
          {/if}
          <a class="d-md-flex w-100 h-100 main-menu__item-link {if $depth === 0}main-menu__item-link--top{else}main-menu__item-link--sub main-menu__item-link--{$depth}{/if} {if $node.children|count}main-menu__item-link--hassubmenu{else}main-menu__item-link--nosubmenu{/if}"
             href="{$node.url}" data-depth="{$depth}"
             {if $node.open_in_new_window} target="_blank" {/if}
             {assign var=_menuitem_label value=$node.label|escape}
             aria-label="{$_menuitem_label}">
...
<div class="d-none d-md-block col-12 header-top__block header-top__block--menu mt-1">
  <nav class="main-menu" id="_desktop_top_menu" role="navigation" aria-label="Top Menu">
